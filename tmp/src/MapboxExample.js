...
const Map3D = () => {
  ...
  const getRouteWithFixedLocation = async (fixedStartLocation, end) => {
    setIsRouting(true);
    mobileLog(`🗺️ 고정 위치 기반 길찾기 시작: [${fixedStartLocation[0].toFixed(6)}, ${fixedStartLocation[1].toFixed(6)}] → [${end[0].toFixed(6)}, ${end[1].toFixed(6)}]`, "info");
    ...
    try {
      ...
      if (data.routes?.length > 0) {
        ...
        mobileLog(`✅ 고정 위치 기준 경로 계산 성공: ${distance}km, ${duration}분`, "success");
  ...
  const handlePinMarkerClick = (coords, feature) => {
    ...
    if (userLocation) {
      ...
      mobileLog(`위치 정보 고정됨: [${fixedStartLocation[0].toFixed(6)}, ${fixedStartLocation[1].toFixed(6)}]`, "success");
      ...
      if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            setUserLocation(userCoords);
            mobileLog(`✅ 위치 정보 재획득 및 고정: [${fixedStartLocation[0].toFixed(6)}, ${fixedStartLocation[1].toFixed(6)}]`, "success");
